...
#region Pause Menu behaviour
        ...
        float angle = Mathf.Atan2(horizontalForward.z, horizontalForward.x) + offsetAngle;
        Vector3 menuHorizontalPosition = new Vector3(
            Mathf.Cos(angle) * cylinderRadius,
            0,
            Mathf.Sin(angle) * cylinderRadius
        );

        Vector3 menuPosition = RightHand.position + menuHorizontalPosition;
